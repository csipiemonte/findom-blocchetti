...
public class NoDecimalFormat {
	...
	public boolean decimalValidate(String inputVal, boolean ammessiNegativi, boolean ammessoZero){
	   ...
	   if(inputVal !=null && StringUtils.isBlank(inputVal) && (inputVal.length()>0)){ // 12,56 non entra
	     valido = false;
	   }
	   	  			
	   if(StringUtils.isEmpty(inputVal) || (!inputVal.matches(regexpr))){ // 12,56 non entra
	     valido = false;
	   }
	   ...
	   if(valido && (!ammessoZero))
	   {
	      ...
	      if(valore.compareTo(new BigDecimal(0))==0){ // 12,56 non entra
	         valido = false;
	      }
	   }
	   return valido;
